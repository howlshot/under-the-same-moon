...
namespace Bouncerock.Terrain
{
											...
											BouncerockEventListener<EvtCameraChanged>
	{
		...
		//Chunks that have been hidden recently but we keep them for a while before actually destroying.

		//NOTE: When chunks are destroyed, everything that happened within them is destroyed as well. If they are to be regenerated thereafter, nothing will remain
		//of their previous state. If you want chunks to have a permanent memory, we'll have to implement hard disk saves, which is not done yet
		Dictionary<Vector2, TerrainChunk> hiddenchunksDictionary = new Dictionary<Vector2, TerrainChunk>();
		...
		public override void _Process(double time)
		{
			...
			if (updateTimer > UpdateFrequency && !updatingChunks)
			{
				updatingChunks = true;
				UpdateChunks();
				updateTimer = 0;
		...
		void HideChunk(Vector2 coord)
		{
			if (chunksDictionary[coord].loading) {return;}//don't hide busy chunks
			if (chunksDictionary.ContainsKey(coord))
			{
				//KeyValuePair<Vector2, TerrainChunk> dflk = chunksDictionary[coord];
				chunksDictionary[coord].SetChunkPendingDestruction();
				

				chunksDictionary.Remove(coord);
			}
		}
		void ShowChunk(Vector2 coord)
		{
			if (hiddenchunksDictionary.ContainsKey(coord))
			{
				chunksDictionary.Add(coord, hiddenchunksDictionary[coord]);

				hiddenchunksDictionary.Remove(coord);
			}
		}

		void DestroyChunk(Vector2 coord)
		{
			hiddenchunksDictionary[coord].Destroy();
			hiddenchunksDictionary.Remove(coord);
		}
